...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos, ImGuiDockNode* dock_node) {
    ...
    if (dock_node) RenderArrowDockMenu(window->DrawList, bb.Min, g.FontSize, text_col);
    else
...
bool ImGui::TempInputScalar(
    ...
    ImGuiInputTextFlags flags =
        ImGuiInputTextFlags_AutoSelectAll | (ImGuiInputTextFlags)ImGuiInputTextFlags_NoMarkEdited;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg) {
    ...
    if (span_all_columns) {
        if (g.CurrentTable) TablePushBackgroundChannel();
    ...
    if (span_all_columns) {
        if (g.CurrentTable) TablePopBackgroundChannel();
        else if (window->DC.CurrentColumns)
            PopColumnsBackground();
    }
...
bool ImGui::ListBox(
    ...
    clipper.Begin(
        items_count,
        GetTextLineHeightWithSpacing()); // We know exactly our line height here so we pass it as a minor optimization,
