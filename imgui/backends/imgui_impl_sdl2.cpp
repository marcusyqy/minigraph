...
bool ImGui_ImplSDL2_ProcessEvent(const SDL_Event* event) {
    ...
    switch (event->type) {
        ...
        case SDL_WINDOWEVENT: {
            // - When capturing mouse, SDL will send a bunch of conflicting LEAVE/ENTER event on every mouse move, but
            // the final ENTER tends to be right.
            // - However we won't get a correct LEAVE event for a captured window.
            // - In some cases, when detaching a window from main viewport SDL may send SDL_WINDOWEVENT_ENTER one frame
            // too late,
            //   causing SDL_WINDOWEVENT_LEAVE on previous frame to interrupt drag operation by clear mouse position.
            //   This is why we delay process the SDL_WINDOWEVENT_LEAVE events by one frame. See issue #5012 for
            //   details.
...
void ImGui_ImplSDL2_SetGamepadMode(
    ...
    if (mode == ImGui_ImplSDL2_GamepadMode_Manual) {
        ...
        for (int n = 0; n < manual_gamepads_count; n++) bd->Gamepads.push_back(manual_gamepads_array[n]);
    } else {
        IM_ASSERT(manual_gamepads_array == nullptr && manual_gamepads_count <= 0);
        bd->WantUpdateGamepadsList = true;
    }
